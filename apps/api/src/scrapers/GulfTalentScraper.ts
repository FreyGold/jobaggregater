...
export class GulfTalentScraper extends BaseScraper {
  ...
  async scrape(): Promise<JobCreateInput[]> {
    ...
    for (const keyword of GT_CATEGORIES) {
      tasks.push(async () => {
        try {
          ...
          if (jobs.length > 0) {
            console.log(
              `[Scraper: ${this.name}] "${keyword}": ${jobs.length} jobs (total: ${allJobs.length})`,
            );
          }
        } catch (err) {
    ...
    for (let page = 1; page <= PAGES; page++) {
      ...
      try {
        ...
        $('div.job-listing, div[class*="job-card"], article[class*="job"]').each((_, el) => {
          ...
          const company = $(el)
            .find('span[class*="company"], div[class*="company"], a[class*="company"]')
            .first()
            .text()
            .trim();
          const location = $(el)
          ...
          if (title && sourceId && !seenIds.has(sourceId)) {
            ...
            found++;
            const snippet = $(el)
              .find('p, div[class*="description"], div[class*="snippet"]')
              .first()
              .text()
              .trim();
            ...
            jobs.push({
              ...
              sourceId,
              sourceName: this.name,
              description: snippet || `${title} at ${company}. Found on GulfTalent.com.`,
  ...
  /**
   * Scrape full job description from individual GulfTalent job page.
  ...
  async scrapeJobDescription(jobUrl: string): Promise<string> {
    try {
      ...
      // GulfTalent job pages selectors
      const selectors = [
        '.job-description',
