...
import { 
  Settings, Check, Copy, RotateCcw, Import, Sun, Moon, 
  ChevronDown, Undo2, Redo2, Wand2, ArrowUpRight, Monitor
} from 'lucide-react';
...
export function ThemeCustomizer() {
  ...
  const ColorInput = ({ label, value, onChange }: { label: string, value: string, onChange: (val: string, final: boolean) => void }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <Settings className={cn("h-[1.2rem] w-[1.2rem] transition-transform duration-300", isOpen && "rotate-90")} />
      </Button>
      ...
      {isOpen && (
        <div className="fixed sm:absolute right-4 sm:right-0 top-16 sm:top-12 z-50 w-[calc(100vw-32px)] sm:w-[380px] rounded-xl border bg-popover p-0 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
           ...
           <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/20">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="h-8 text-[11px] font-semibold bg-background" onClick={() => pushToHistory(DEFAULT_CONFIG)}>
                <RotateCcw className="h-3.5 w-3.5 mr-1.5" /> Reset
              </Button>
              <Button variant="secondary" size="icon" className="h-8 w-8" onClick={() => setIsOpen(false)}>
          ...
          <div className="p-5 max-h-[75vh] overflow-y-auto custom-scrollbar">
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <Label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">Appearance</Label>
                <div className="flex p-1 bg-muted rounded-lg gap-1 border border-border/10">
                  <Button
                    variant={activeTheme === 'light' ? "secondary" : "ghost"}
                    size="sm"
                    className={cn("flex-1 h-8 rounded-md transition-all", activeTheme === 'light' && "bg-background shadow-sm")}
                  ...
                  <Button
                    ...
                    size="sm"
                    className={cn("flex-1 h-8 rounded-md transition-all", activeTheme === 'dark' && "bg-background shadow-sm")}
                  ...
                  <Button
                    ...
                    size="sm"
                    className={cn("flex-1 h-8 rounded-md transition-all", activeTheme === 'system' && "bg-background shadow-sm")}
              ...
              {/* Theme Presets */}
              <div className="space-y-3">
                <Label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70">Theme</Label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Select 
                      ...
                      onValueChange={(val) => {
                        ...
                        if (preset) pushToHistory({ ...DEFAULT_CONFIG, ...preset, theme: val });
                      }}
                    >
                  ...
                  <Button variant="outline" size="icon" className="h-9 w-9" title="Randomize" onClick={() => {
              ...
              <Tabs defaultValue="colors" className="w-full">
                <TabsList className="w-full h-9 bg-muted/50 p-1">
                  ...
                  <TabsTrigger value="layout" className="text-xs">Other</TabsTrigger>
                </TabsList>
                
                <TabsContent value="colors" className="mt-4">
                  <div className="grid grid-cols-2 gap-2 pb-2">
                    {['primary', 'background', 'foreground', 'card', 'border', 'secondary', 'muted', 'accent'].map((key) => (
                      <ColorInput 
                        key={key} 
                        label={key} 
                ...
                <TabsContent value="typography" className="mt-4 space-y-4">
                  {[
                    ...
                    <div key={font.key} className="space-y-2">
                      ...
                      <Select 
                        value={config[font.key as keyof ThemeConfig]}
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="Geist, sans-serif" className="text-xs">Geist</SelectItem>
                          <SelectItem value="Inter, sans-serif" className="text-xs">Inter</SelectItem>
                          <SelectItem value="system-ui, sans-serif" className="text-xs">System</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  ))}
                  ...
                  {[
                    ...
                    <div key={s.key} className="space-y-3">
                      <div className="flex justify-between items-center">
                        <Label className="text-[10px] uppercase text-muted-foreground/70">{s.label}</Label>
                        <span className="text-[10px] font-mono text-muted-foreground">{config[s.key as keyof ThemeConfig]}</span>
                      </div>
                      <input
                        type="range"
                        min={s.min}
                        ...
                        value={parseFloat(config[s.key as keyof ThemeConfig] as string)}
                        onChange={(e) => updateConfigValue(s.key as keyof ThemeConfig, e.target.value + s.unit, false)}
                        onMouseUp={(e) => updateConfigValue(s.key as keyof ThemeConfig, (e.target as HTMLInputElement).value + s.unit, true)}
                      />
                    </div>
                  ))}
                </TabsContent>
            ...
            <div className="mt-8 pt-4 border-t flex flex-col gap-2">
              <Button size="sm" className="w-full h-9 font-bold" onClick={() => navigator.clipboard.writeText(JSON.stringify(config, null, 2))}>
